...
void UCustomAbilitySystemComponent::ChangeLevel_Implementation(const float DesiredLevel, const bool bInitialization /* = false */)
{
	...
	if (const AGridV1CharacterBase* const CharacterBase = GetCharacterBaseAvatar())
	{
...
AGridV1CharacterBase* UCustomAbilitySystemComponent::GetCharacterBaseAvatar() const
{
	return Cast<AGridV1CharacterBase>(GetAvatarActor_Direct());
}
...
FActiveGameplayEffectHandle UCustomAbilitySystemComponent::ApplyGameplayEffectSpecToTarget(const FGameplayEffectSpec& GameplayEffect, UAbilitySystemComponent* Target, FPredictionKey PredictionKey)
{
	if (Target->GetOwner())
	{
		UCustomAbilitySystemBPLibrary::SetTargetOnGameplayEffectContextFromSpec(const_cast<FGameplayEffectSpec&>(GameplayEffect), Target->GetOwner());
	}
